...
export const authService = {
  ...
  async signInWithGoogle(credential: string): Promise<User> {
    ...
    if (existing) {
      ...
      console.log('🔄 [auth.ts] Updating existing user:', existing.email);
      console.log('🔄 [auth.ts] User ID:', existing.id);
      console.log('🔄 [auth.ts] Update fields:', {
      ...
      if (error) {
        console.error('❌ [auth.ts] Database update error:', error);
        console.error('❌ [auth.ts] Error code:', error.code);
        console.error('❌ [auth.ts] Error message:', error.message);
        console.error('❌ [auth.ts] Error details:', JSON.stringify(error, null, 2));
        console.error('❌ [auth.ts] This may be an RLS policy issue - check Supabase logs');
        
        // If update fails, try to return existing user data (better than failing completely)
        console.warn('⚠️ [auth.ts] Update failed, returning existing user data as fallback');
        const subscription = await getSubscriptionStatus(existing.id);
        return {
          id: existing.id,
          email: existing.email,
          fullName: existing.full_name || payload.name,
          avatar: existing.avatar_url || payload.picture,
          ...subscription
        };
      }
      
      console.log('✅ [auth.ts] User update successful');
